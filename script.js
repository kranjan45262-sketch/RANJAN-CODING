...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function openCart() {
    ...
    await loadCustomerProfile(); $('#order-name').value = currentProfile ? currentProfile.full_name : ''; $('#order-phone').value = currentProfile ? currentProfile.phone : ''; $('#order-modal').classList.add('is-open'); $('#order-modal').setAttribute('aria-hidden', 'false'); document.body.classList.add('modal-open'); renderCart();
  }
  ...
  $('#saved-address-select').addEventListener('change', useSelectedAddress);
  ...
  $('#checkout-form').addEventListener('submit', async function (event) {
    ...
    if (remoteClient) {
      ...
      if (session.data.user) {
        const saved = await remoteClient.from('orders').insert({ customer_id: session.data.user.id, customer_name: order.name, customer_phone: order.phone, order_type: order.type, address_id: $('#order-address-id').value || null, address: order.address, note: order.note, payment_method: order.payment, transaction_id: order.transaction, total: order.total }).select('id').single();
  ...
  function renderSavedAddresses() {
    const list = $('#saved-addresses'); const select = $('#saved-address-select');
    list.innerHTML = addresses.length ? addresses.map(function (address) { return '<div class="saved-address-card"><strong>' + escapeHtml(address.label) + '</strong><p>' + escapeHtml(address.address_line) + ', ' + escapeHtml(address.city) + ' - ' + escapeHtml(address.pincode) + '</p><button type="button" class="text-button edit-address" data-id="' + escapeHtml(address.id) + '">Edit</button><button type="button" class="text-button delete-address" data-id="' + escapeHtml(address.id) + '">Delete</button></div>'; }).join('') : '<p class="account-help">Abhi koi saved address nahi hai.</p>';
    select.innerHTML = '<option value="new">Use a new address</option>' + addresses.map(function (address) { return '<option value="' + escapeHtml(address.id) + '">' + escapeHtml(address.label) + ' — ' + escapeHtml(address.city) + ' ' + escapeHtml(address.pincode) + '</option>'; }).join('');
  }
  async function loadCustomerProfile() {
    if (!remoteClient) return;
    const userResult = await remoteClient.auth.getUser(); if (!userResult.data.user) return;
    const profileResult = await remoteClient.from('profiles').select('id, full_name, phone, role, address_line, city, pincode').eq('id', userResult.data.user.id).single();
    if (profileResult.data) { currentProfile = profileResult.data; $('#profile-name').value = currentProfile.full_name || ''; $('#profile-phone').value = currentProfile.phone || ''; $('#profile-address-line').value = currentProfile.address_line || ''; $('#profile-city').value = currentProfile.city || ''; $('#profile-pincode').value = currentProfile.pincode || ''; }
    const addressesResult = await remoteClient.from('addresses').select('id, label, address_line, city, pincode, is_default').order('created_at', { ascending: false }); addresses = addressesResult.data || []; renderSavedAddresses();
  }
  function useSelectedAddress() { const id = $('#saved-address-select').value; const address = addresses.find(function (item) { return item.id === id; }); $('#order-address-id').value = address ? address.id : ''; if (address) $('#order-address').value = address.address_line + ', ' + address.city + ' - ' + address.pincode; }
  ...
  function openAuth() { $('#auth-modal').classList.add('is-open'); $('#auth-modal').setAttribute('aria-hidden', 'false'); document.body.classList.add('modal-open'); if (activeCustomer && remoteClient) { $('#login-form').hidden = true; $('#signup-form').hidden = true; $('.auth-tabs').hidden = true; $('.auth-intro').hidden = true; $('#customer-profile-panel').hidden = false; loadCustomerProfile(); } }
  ...
  document.querySelectorAll('.auth-tab').forEach(function (button) { button.addEventListener('click', function () { const signup = button.dataset.authView === 'signup'; document.querySelectorAll('.auth-tab').forEach(function (item) { item.classList.toggle('active', item === button); }); $('#login-form').hidden = signup; $('#signup-form').hidden = !signup; $('#customer-profile-panel').hidden = true; $('.auth-tabs').hidden = false; $('.auth-intro').hidden = false; $('#auth-note').textContent = ''; }); });
  $('#signup-form').addEventListener('submit', async function (event) { event.preventDefault(); const customer = { name: $('#signup-name').value.trim(), email: $('#signup-email').value.trim().toLowerCase(), phone: $('#signup-phone').value.trim(), passwordHash: await hashPassword($('#signup-password').value), created: new Date().toLocaleString('en-IN') }; const addressLine = $('#signup-address-line').value.trim(); const city = $('#signup-city').value.trim(); const pincode = $('#signup-pincode').value.trim(); if (pincode && !/^\d{6}$/.test(pincode)) { $('#auth-note').textContent = 'Pincode 6 digits ka hona chahiye.'; return; } if (remoteClient) { const result = await remoteClient.auth.signUp({ email: customer.email, password: $('#signup-password').value, options: { data: { full_name: customer.name, phone: customer.phone, address_line: addressLine, city: city, pincode: pincode } } }); if (result.error) { $('#auth-note').textContent = result.error.message; return; } if (!result.data.session) { $('#auth-note').textContent = 'Account create ho gaya. Email verify karke phir login karein.'; return; } } else { if (customers.some(function (item) { return item.email === customer.email; })) { $('#auth-note').textContent = 'Is email se account pehle se bana hua hai. Login karein.'; return; } customers.push(customer); saveCustomers(); } setActive(customer); renderCustomers(); $('#auth-note').textContent = 'Account create ho gaya. Ab aap order kar sakte hain.'; setTimeout(closeAuth, 1000); });
  $('#login-form').addEventListener('submit', async function (event) { event.preventDefault(); const email = $('#login-email').value.trim().toLowerCase(); let found; if (remoteClient) { const result = await remoteClient.auth.signInWithPassword({ email: email, password: $('#login-password').value }); if (result.error) { $('#auth-note').textContent = result.error.message; return; } const profile = await remoteClient.from('profiles').select('full_name, phone').eq('id', result.data.user.id).single(); found = { name: profile.data ? profile.data.full_name : email, email: email, phone: profile.data ? profile.data.phone : '' }; } else { const passwordHash = await hashPassword($('#login-password').value); found = customers.find(function (item) { return item.email === email && item.passwordHash === passwordHash; }); if (!found) { $('#auth-note').textContent = 'Email ya password galat hai.'; return; } } setActive(found); await syncRemoteData(); await loadCustomerProfile(); $('#auth-note').textContent = 'Login successful.'; setTimeout(closeAuth, 600); });
  $('#profile-form').addEventListener('submit', async function (event) { event.preventDefault(); if (!remoteClient || !currentProfile) return; const pincode = $('#profile-pincode').value.trim(); if (pincode && !/^\d{6}$/.test(pincode)) { $('#profile-note').textContent = 'Pincode 6 digits ka hona chahiye.'; return; } const result = await remoteClient.from('profiles').update({ full_name: $('#profile-name').value.trim(), phone: $('#profile-phone').value.trim(), address_line: $('#profile-address-line').value.trim(), city: $('#profile-city').value.trim(), pincode: pincode }).eq('id', currentProfile.id); $('#profile-note').textContent = result.error ? result.error.message : 'Profile saved.'; if (!result.error) await loadCustomerProfile(); });
  $('#address-form').addEventListener('submit', async function (event) { event.preventDefault(); if (!remoteClient) return; const pincode = $('#address-pincode').value.trim(); if (!/^\d{6}$/.test(pincode)) { $('#profile-note').textContent = 'Pincode 6 digits ka hona chahiye.'; return; } const payload = { label: $('#address-label').value.trim() || 'Home', address_line: $('#address-line').value.trim(), city: $('#address-city').value.trim(), pincode: pincode, updated_at: new Date().toISOString() }; const result = this.dataset.editId ? await remoteClient.from('addresses').update(payload).eq('id', this.dataset.editId) : await remoteClient.from('addresses').insert(Object.assign(payload, { customer_id: (await remoteClient.auth.getUser()).data.user.id })); if (result.error) $('#profile-note').textContent = result.error.message; else { delete this.dataset.editId; this.reset(); this.querySelector('button').textContent = 'Save address'; $('#profile-note').textContent = 'Address saved.'; await loadCustomerProfile(); } });
  $('#saved-addresses').addEventListener('click', async function (event) { const id = event.target.dataset.id; if (!id || !remoteClient) return; const address = addresses.find(function (item) { return item.id === id; }); if (event.target.classList.contains('edit-address')) { $('#address-label').value = address.label; $('#address-line').value = address.address_line; $('#address-city').value = address.city; $('#address-pincode').value = address.pincode; $('#address-form').dataset.editId = id; $('#address-form button').textContent = 'Update address'; } if (event.target.classList.contains('delete-address') && window.confirm('Is saved address ko delete karna hai?')) { await remoteClient.from('addresses').delete().eq('id', id); await loadCustomerProfile(); } });
  $('#logout-button').addEventListener('click', async function () { if (remoteClient) await remoteClient.auth.signOut(); setActive(null); currentProfile = null; addresses = []; $('#customer-profile-panel').hidden = true; $('.auth-tabs').hidden = false; $('.auth-intro').hidden = false; $('#login-form').hidden = false; $('#signup-form').hidden = true; setAdminMode(false); closeAuth(); });
  ...
  updateCounts(); renderOrders(); renderCustomers(); setAdminMode(false); loadProducts(); if (activeCustomer) { setActive(activeCustomer); syncRemoteData(); loadCustomerProfile(); }
});
